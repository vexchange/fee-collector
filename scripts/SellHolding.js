...
import { FEE_COLLECTOR_WVET_ADDRESS, WVET_ADDRESS, PRIVATE_KEY, MAINNET_NODE_URL } from "./config.js";
...
async function SellHolding() {
    ...
    const lFeeCollectorContract = lProvider.thor.account(FEE_COLLECTOR_WVET_ADDRESS);
    ...
    for (const lToken of lTokens.keys()) {
        ...
        const lTokenBalance = await GetERC20Balance(lToken, FEE_COLLECTOR_WVET_ADDRESS, lProvider);
