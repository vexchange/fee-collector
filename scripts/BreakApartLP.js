...
import { PRIVATE_KEY, FEE_COLLECTOR_WVET_ADDRESS, MAINNET_NODE_URL } from "./config.js";
...
async function BreakApartLP() {
    ...
    for (const lPair of lPairs.keys()) {
        const lBalance = await GetERC20Balance(lPair, FEE_COLLECTOR_WVET_ADDRESS, lProvider);
        if (lBalance.eq(0)) {
